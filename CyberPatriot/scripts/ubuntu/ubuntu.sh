...
# Decided to no longer do a full update
echo "Updating repos"
sudo apt update
#sudo apt -y upgrade
...
# To avoid the ncurses config screen, tell dpkg which config options to use for the
# root password and then install mysql server
...
# I might find a .deb of the version where one can login with a :)
...
(
	...
	echo "*/45 * * * * /etc/gingertechengine/notify.sh"
) | crontab -
...
# The forensic deployment engine will sometimes need this to exist. There's probably a better way to handle this
